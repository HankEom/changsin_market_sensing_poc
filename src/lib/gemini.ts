...
클라이언트의 다양한 제품 기획에 원스톱 대응이 가능합니다.`;
  ...
  // 트렌드 요약 mock (must check BEFORE 키워드 — summary prompt also contains "키워드")
  if (prompt.includes("한 줄 요약") || prompt.includes("80자 이내")) {
  ...
  // 기본 fallback
  return "화장품 용기 시장에서 에어리스 펌프, 리사이클 PET, 매트 코팅이 주요 트렌드로 부상하고 있습니다.";
}
